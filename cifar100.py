...
parser = argparse.ArgumentParser(description="PyTorch 0 Training")
parser.add_argument(
    "--model",
    default="2l_lnn",
    type=str,
    help="model, one of bn_1l_lnn, 1l_lnn, 2l_lnn, 3l_lnn, 2l_fc, 3l_fc, resnet_18",
)
parser.add_argument("--lr", default=1e-3, type=float, help="learning rate")
parser.add_argument("--bsize", default=128, type=int, help="minibatch size")
parser.add_argument(
    "--sampling", default="RR", type=str, help="sampling type, one of RR, SS or SGD"
)
...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="use EMA to estimate population stats",
)
parser.add_argument(
    "--dn",
    dest="dn",
...
parser.add_argument(
    "--num_classes", default=100, type=int, help="How many classes to use in CIFAR10"
)
args: Namespace = parser.parse_args()

writer = SummaryWriter(
    comment=f"cifar100_shuffling_{args.sampling}_lr_{args.lr}_epoch_{args.epoch}_width_{args.width}_bsize_{args.bsize}_momen_{args.momen}_ema_{args.ema}_model_{args.model}_{args.suffix}"
)
...
class LNN1(nn.Module):  # aka the linear model
    def __init__(self, input_size=3 * 32 * 32):
        ...
        self.linear_1 = nn.Linear(input_size, 100, bias=False)
        self.bn_1 = nn.BatchNorm1d(
            input_size, affine=not args.freeze_gamma, track_running_stats=args.ema
        )
        self.bn_1_no_affine = nn.BatchNorm1d(
            input_size, affine=False, track_running_stats=args.ema
        )
...
class BNLNN1(nn.Module):  # aka linear model
    def __init__(self):
        ...
        self.flatten = nn.Flatten()
        self.linear_1 = nn.Linear(3 * 32 * 32, 100, bias=False)
        self.bn_1 = nn.BatchNorm1d(100, track_running_stats=args.ema)
        if not args.bn:
            self.linear_relu_stack = nn.Sequential(self.linear_1)
        else:
...
class LNN2(nn.Module):
    def __init__(self, input_size=3 * 32 * 32):
        ...
        self.flatten = nn.Flatten()
        self.linear_1 = nn.Linear(args.width, 100, bias=False)
        self.linear_2 = nn.Linear(input_size, args.width, bias=False)

        self.bn_1 = nn.BatchNorm1d(
            args.width, affine=not args.freeze_gamma, track_running_stats=args.ema
        )
        self.bn_1_no_affine = nn.BatchNorm1d(
            args.width, affine=False, track_running_stats=args.ema
        )
...
class FC2(nn.Module):
    def __init__(self, input_size=3 * 32 * 32):
        ...
        self.linear_1 = nn.Linear(args.width, 100, bias=True)
...
class FC3(nn.Module):
    def __init__(self):
        ...
        if not args.bn:
            self.linear_relu_stack = nn.Sequential(
                self.linear_3, nn.ReLU(), self.linear_2, nn.ReLU(), self.linear_1
            )
            self.feature_stack = nn.Sequential(
                self.linear_3,
                nn.ReLU(),
...
def calcloss(
    ...
    with torch.no_grad():
        for X, y in train_eval_dataloader:
            ...
            pred = model(X)
            loss = loss_fn(pred, y)
            train_loss += loss.item() * train_eval_batch_size
    ...
    writer.add_scalar("fb loss/train", train_loss, epochidx)
    ...
    with torch.no_grad():
        for X, y in test_eval_dataloader:
            ...
            if epochidx % 200 == 0 and args.model != "resnet_18":
                ...
                if d == 2:
                    fig, ax = plot_convex_hulls(
                        learned_features.cpu().numpy(), y.cpu().numpy()
                    )
    ...
    writer.add_scalar("fb loss/test", test_loss, epochidx)
    writer.add_scalar("fb acc/test", test_correct, epochidx)
    ...
    if args.model != "resnet_18":
        ...
        if epochidx % 100 == 0:
            for name, param in model.named_parameters():
                ...
                writer.add_scalar(
                    f"{name} norm normalized by size",
                    param.norm() / param_size,
                    epochidx,
                )
            ...
            M = W @ Gamma
            writer.add_scalar(
                f"M norm normalized by size",
                M.norm() / np.sqrt(torch.numel(M)),
                epochidx,
            )
        ...
        M = model.fc.weight
        writer.add_scalar(
            f"M norm normalized by size", M.norm() / np.sqrt(torch.numel(M)), epochidx
        )
        writer.add_scalar(f"M norm unnormalized", M.norm(), epochidx)
    if save:
...
if __name__ == "__main__":
    ...
    train_eval_batch_size = (
        args.bsize if args.model == "resnet_18" else len(training_data)
    )
    ...
    train_plot_dataloader = DataLoader(
        training_data, batch_size=train_eval_batch_size, num_workers=1
    )
    test_plot_dataloader = DataLoader(
        test_data, batch_size=test_eval_batch_size, num_workers=1
    )
