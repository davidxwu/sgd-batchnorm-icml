...
parser.add_argument('--charlie_dataset', default=False, action='store_true', help='whether to use charlie dataset')
parser.add_argument('--toy_dataset', default=False, action='store_true', help='whether to use toy dataset')
...
writer = SummaryWriter(comment=f'_toy_data_shuffling_{args.sampling}_lr_{args.lr}_epoch_{args.epoch}_model_{args.model}_num_datapoints_{args.num_datapoints}_bsize_{args.bsize}_bn_{args.bn}_freeze_{args.freeze_gamma}_seed_{args.seed}_charlie_dataset_{args.charlie_dataset}_{args.suffix}')
...
class LNN1(nn.Module):  # aka the linear model
    def __init__(self, input_size=2):
        ...
        self.bn_1 = nn.BatchNorm1d(input_size, affine=not args.freeze_gamma, eps=0, track_running_stats=False)
        self.bn_1_no_affine = nn.BatchNorm1d(input_size, affine=False, eps=0, track_running_stats=False)
...
class ToyDataset(Dataset):
    def __init__(self, m):
        B = np.hstack((np.linspace(-2.2, -1.8, m).reshape(m, 1), np.linspace(1.8, 2.2, m).reshape(m, 1)))
...
class BlobsDataset(Dataset):
    def __init__(self, num_samples=12, num_features=2, centers=2, transform=None, target_transform=None, random_state=None):
        self.X, self.y = make_blobs(n_samples=num_samples, n_features=num_features, centers=centers, random_state=args.seed if random_state is None else random_state)
...
class CharlieDataset(Dataset):
    def __init__(self, transform=None, target_transform=None):
        self.X1 = torch.Tensor([[1, 0], [0, 1], [0.01, -0.01], [-0.01, 0.01], [0.01, -0.01], [-0.01, 0.01]])
        self.X2 = torch.Tensor([[0.01, -0.01], [-0.01, 0.01], [1, 0], [0, 1], [-0.01, 0.01], [0.01, -0.01]])
        self.X3 = torch.Tensor([[0.01, -0.01], [-0.01, 0.01], [0.01, -0.01], [-0.01, 0.01], [1, 0], [0, 1]])
...
def plot_convex_hulls(X, y, pos_color='b', neg_color='r', pos_label='Positive', neg_label='Negative', ax=None):
    positive_labels = y == 1
    positive_features = X[positive_labels, :]
    negative_features = X[~positive_labels, :]
    if ax is None:
        fig, ax = plt.subplots()
    ax.plot(positive_features[:,0], positive_features[:,1], '+', color=pos_color, markersize=10, mew=3, alpha=0.8, label=pos_label)
    ax.plot(negative_features[:,0], negative_features[:,1], 'o', color=neg_color, alpha=0.8, label=neg_label)
    ...
    for simplex in positive_hull.simplices:
        ax.plot(positive_features[simplex, 0], positive_features[simplex, 1], color=pos_color, linestyle='--', alpha=0.5)
    for simplex in negative_hull.simplices:
        ax.plot(negative_features[simplex, 0], negative_features[simplex, 1], color=neg_color, linestyle='--', alpha=0.5)
    ax.legend()
...
def calcloss(train_eval_dataloader, test_eval_dataloader, model, loss_fn, epochidx, savefilename, save=True):
    ...
    with torch.no_grad():
        ...
        if epochidx % 200 == 0:
            ...
            fig, ax = plt.subplots()
            plot_convex_hulls(learned_SS_features, y_SS, pos_color='purple', neg_color='green', pos_label='Positive (SS)', neg_label='Negative (SS)', ax=ax)
            ax.set_title(f'2d visualization of SS features at epoch {epochidx}')

            W = model.linear_1.weight
            # print(W)
            #     W = param
            plt.axline((0, 0), slope=-W[0, 0].item()/W[0, 1].item(), color='green', linestyle=':', label=r'$(v_{\pi}^*)^\top x = 0$')
    ...
    with torch.no_grad():
        for X, y in test_eval_dataloader:
            ...
            if epochidx % 200 == 0:
                ...
                plot_convex_hulls(learned_GD_features, y.cpu().numpy(), pos_color='blue', neg_color='red', pos_label='Positive (GD)', neg_label='Negative (GD)', ax=ax)
    ...
    if epochidx % 100 == 0:
        ...
        # M = W if args.freeze_gamma else W * Gamma
 #       print(W, Gamma)
  #      print(M.shape)
        # if torch.numel(M) < 10:
...
if __name__ == '__main__':
    ...
    train_plot_dataloader = DataLoader(training_data, batch_size=train_eval_batch_size, num_workers=2)
    ...
    if args.resume <= 0:
        ...
        for t in range(args.epoch):
            ...
            calcloss(train_dataloader, train_plot_dataloader, model, loss_fn, t + 1,
        ...
        calcloss(train_dataloader, train_plot_dataloader, model, loss_fn, args.resume,
