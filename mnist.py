...
parser.add_argument(
    "--num_classes", default=10, type=int, help="How many classes to use in MNIST"
)
args: Namespace = parser.parse_args()

writer = SummaryWriter(
    comment=f"_mnist_shuffling_{args.sampling}_lr_{args.lr}_epoch_{args.epoch}_width_{args.width}_bsize_{args.bsize}_momen_{args.momen}_ema_{args.ema}_model_{args.model}_{args.suffix}"
)
...
class LNN3(nn.Module):
    def __init__(self):
        ...
        self.linear_3 = nn.Linear(1 * 28 * 28, args.width, bias=False)
...
class FC3(nn.Module):
    def __init__(self):
        ...
        self.linear_3 = nn.Linear(1 * 28 * 28, args.width, bias=True)
...
if __name__ == "__main__":
    ...
    if args.model == "resnet_18":
        transform = transforms.Compose(
            [transforms.Lambda(lambda x: x.convert("RGB")), weights.transforms()]
        )
    elif args.dn:
        transform = transforms.Compose(
            [
